...
ตั้งแต่ 2569-09-17 เปลี่ยนจากรหัสผ่านตัวเดียวร่วมกัน เป็น **บัญชีผู้ดูแลรายคน** อีเมล + รหัสผ่าน

เปิด `https://timework.scriptbin.dev/` จะถูกพาไปหน้า `login` ให้กรอกอีเมลและรหัสผ่าน ติ๊ก จดจำการเข้าสู่ระบบ 30 วัน ไว้ได้ ถ้ากรอกรหัสผิด 5 ครั้งติดกัน บัญชีจะถูกล็อก 15 นาที

### บัญชีผู้ดูแลระบบคนแรก

ระบบสร้างให้อัตโนมัติจาก `ADMIN_BOOTSTRAP_EMAIL` ใน `.env` ตอนเริ่มระบบครั้งแรก แล้วพิมพ์ลิงก์ตั้งรหัสผ่านลง log อ่านได้ด้วย

```bash
cd /home/aon/timework-line
docker compose logs api | grep 'สร้างบัญชีผู้ดูแลระบบคนแรก'
```

ลิงก์ใช้ได้ 24 ชั่วโมงและใช้ได้ครั้งเดียว ถ้าเลยเวลาแล้วให้ลบบัญชีนั้นออกจากตาราง `admins` แล้วรีสตาร์ต ระบบจะสร้างให้ใหม่พร้อมลิงก์ใหม่

**ตั้งใจไม่ให้คนแรกที่สมัครกลายเป็นผู้ดูแลระบบอัตโนมัติ** เพราะเว็บนี้เปิดอยู่บนอินเทอร์เน็ตจริง ใครก็เปิดหน้าสมัครได้

### สมัครใช้งานและอนุมัติ

คนใหม่กด ส่งคำขอใช้งาน ที่หน้า login กรอกชื่อ อีเมล รหัสผ่าน และเลือกสิทธิ์ที่อยากได้ คำขอจะอยู่ในสถานะรออนุมัติ **เข้าระบบยังไม่ได้จนกว่าจะมีคนอนุมัติ**

สิทธิ์ที่ผู้สมัครเลือกเป็นเพียงคำขอ บทบาทจริงคือค่าที่ผู้อนุมัติกด แก้ได้ในช่องก่อนกดอนุมัติ ดูคิวที่เมนู **ผู้ดูแลระบบ**

### สี่บทบาท

| บทบาท | ใช้ทำอะไรได้ |
| --- | --- |
| ผู้ดูแลระบบ | ทุกอย่าง รวมถึงอนุมัติคำขอและแก้ตารางสิทธิ์ |
| ฝ่ายบุคคล | พนักงาน กะ ตาราง การลา บันทึกเวลา ลืมลงเวลา อนุมัติ OT · เห็นค่าตอบแทนแต่แก้ไม่ได้ · ไม่เห็นหน้าเงินที่เบิก |
| การเงิน | สามเมนูการเงินและตั้งค่ากฎคำนวณ · เห็นข้อมูลพนักงานแต่แก้ไม่ได้ |
| หัวหน้าแผนก | เห็นเฉพาะพนักงานในแผนกของตัวเอง · ไม่เห็นเรื่องเงินเลย |

แก้ว่าบทบาทไหนเห็นหรือแก้เมนูใดได้ที่เมนู **สิทธิ์ตามบทบาท** กดที่ช่องเพื่อวนค่า สิทธิ์ของผู้ดูแลระบบถูกล็อกไว้แก้ไม่ได้ กันการตั้งค่าพลาดแล้วล็อกตัวเองออก

ระดับที่มีคำว่า (ทีม) หมายถึงเห็นเฉพาะแผนกของตัวเอง เซิร์ฟเวอร์เป็นคนใส่แผนกให้เอง **ไม่รับค่าจาก client** ยิง `?departmentId=` ข้ามแผนกไม่ได้

### ลืมรหัสผ่าน

กด ลืมรหัสผ่าน ที่หน้า login ระบบส่งลิงก์ตั้งรหัสใหม่ **ทาง LINE** ของบัญชีนั้น (ระบบนี้ไม่มีช่องทางอีเมล) ลิงก์ใช้ได้ 30 นาทีและใช้ได้ครั้งเดียว ตั้งรหัสใหม่แล้วอุปกรณ์อื่นที่ค้างอยู่จะถูกออกจากระบบทั้งหมด

บัญชีที่ยังไม่ได้ผูก LINE จะส่งไม่ได้ ผูกได้ที่เมนู ผู้ดูแลระบบ กดปุ่ม ผูกบัญชี แล้วเลือกรหัสพนักงานที่ผูก LINE ไว้แล้ว ถ้าส่งไม่ได้ ลิงก์จะไปอยู่ใน log ของเซิร์ฟเวอร์แทน

### เรียกผ่านสคริปต์

`ADMIN_API_KEY` ยังใช้ได้เหมือนเดิม **เฉพาะงานอัตโนมัติ** โดยมีสิทธิ์เท่าผู้ดูแลระบบ

```bash
curl -H "x-admin-key: $ADMIN_API_KEY" https://timework.scriptbin.dev/api/employees
```

เส้นทางที่เปิดไว้โดยไม่ต้องยืนยันตัวตน

- `/health` เพื่อให้ระบบ monitoring ตรวจสถานะได้
- `/webhooks/line` เพราะตรวจ HMAC signature จาก LINE อยู่แล้ว
- `/login` `/reset` และ `/auth/*` เพราะต้องเข้าถึงได้ก่อนยืนยันตัวตน
- `/liff*` เพราะยืนยันด้วย access token ของ LINE แทน
...
เปิดสิทธิ์ที่การ์ด **สิทธิ์ส่งรายการผ่าน LINE** ทางขวาของหน้า รายการค่าใช้จ่าย กดปุ่มข้างชื่อเพื่อสลับระหว่าง ส่งได้ กับ ปิดอยู่ มีผลทันทีไม่ต้องกดบันทึก ค่าเริ่มต้นคือปิดทุกคน ถ้ายังไม่มีใครได้สิทธิ์ ทุกคนจะเปิดหน้าค่าใช้จ่ายใน LINE ไม่ได้

คนที่ยังไม่ผูกบัญชี LINE จะมีป้ายเตือนในการ์ด ต่อให้เปิดสิทธิ์ให้ก็ยังใช้เมนูไม่ได้จนกว่าจะผูกบัญชีก่อน แก้เป็นรายคนจากหน้า พนักงาน ได้เหมือนเดิม
...
**แนบรูปใบเสร็จได้** กดช่อง รูปใบเสร็จ แล้วถ่ายรูปหรือเลือกจากคลังภาพ ระบบย่อรูปให้ในเครื่องก่อนส่ง (ด้านยาวสุดเหลือ 1600 จุด บีบเป็น JPEG) พนักงานจึงไม่เปลืองเน็ตและส่งได้เร็วแม้ถ่ายจากมือถือความละเอียดสูง มีตัวอย่างรูปให้ดูก่อนส่งและกดเอารูปออกได้ รับเฉพาะ JPG กับ PNG ขนาดหลังย่อไม่เกิน 3 MB แนบรูปหรือไม่ก็ได้ ไม่ใช่ของบังคับ

รายการที่มีรูปจะมีปุ่ม ดูใบเสร็จ ทั้งในหน้าของพนักงานและในตารางของผู้ดูแล พนักงานเปิดดูได้เฉพาะรูปของรายการตัวเองเท่านั้น ลบรายการแล้วไฟล์รูปจะถูกลบตามไปด้วย

ผู้ดูแลตรวจที่การ์ด รายการค่าใช้จ่ายที่พนักงานส่งมา ในหน้า ค่าจ้าง กดปุ่ม ดูใบเสร็จ ในช่องรายละเอียดเพื่อเปิดรูปเต็มจอ หรือกด เปิดในแท็บใหม่ เพื่อดูขนาดจริง
...
เมนู ค่าจ้าง เดิมถูกแยกเป็นสามหน้าใต้หัวข้อ **การเงิน** ในเมนูซ้าย ทั้งสามหน้าใช้ **ตัวเลือกเดือนร่วมกัน** ที่มุมขวาบน กดลูกศรซ้ายขวาเปลี่ยนเดือน แล้วทุกหน้าเปลี่ยนตามพร้อมกัน ไม่ต้องกรอกช่วงวันที่เองอีก

เมนู รายการค่าใช้จ่าย มีตัวเลขสีเหลืองบอกจำนวนรายการที่ยังรอตรวจทั้งเดือน

### หน้า เงินค่าตอบแทน

บนสุดเป็นการ์ดสรุปสี่ใบ ค่าแรงรวม · ค่า OT ที่อนุมัติ · หักสาย/ขาดงาน · คงเหลือรอจ่าย ถัดลงมาเป็นตารางรายคน กรองตามแผนกหรือรายบุคคลได้ และกด **ส่งออก Excel** เพื่อดาวน์โหลดไฟล์ CSV ที่เปิดใน Excel ได้

| คอลัมน์ | ที่มา |
| --- | --- |
| ทำงาน ขาด ลา | นับจากตารางและการสแกนในเดือนนั้น |
...
| ค่าใช้จ่ายอนุมัติ | ยอดค่าใช้จ่ายที่อนุมัติในเดือนนั้น เป็นเงินที่พนักงานสำรองจ่ายไป จึงบวกคืนให้ |
| ยอดสุทธิ | ค่าแรง + OT − หัก + ค่าใช้จ่ายที่อนุมัติ |
| เบิก และ จ่ายแล้ว | รวมรายการเบิกและจ่ายที่วันที่อยู่ในเดือนเดียวกัน |
...
การ์ด **อนุมัติ OT** อยู่ล่างสุดของหน้านี้ เพราะ OT ที่อนุมัติเป็นตัวตั้งของค่า OT ในตารางด้านบน

### หน้า เงินที่เบิก

การ์ดสรุปสามใบ เบิกล่วงหน้าเดือนนี้ · จ่ายเงินแล้ว · จำนวนรายการ ด้านซ้ายเป็นตารางรายการ ด้านขวาเป็นฟอร์มบันทึก

ฟอร์มเรียงจากบนลงล่าง เลือกพนักงาน · วันที่กับจำนวนเงินอยู่แถวเดียวกัน · เลือกประเภทด้วย**ปุ่มสองอัน** เบิกล่วงหน้า (เหลือง) หรือ จ่ายเงิน (เขียว) สีเดียวกับป้ายในตาราง · ช่องทางรับเงิน พิมพ์เองได้หรือเลือกจากรายการที่แนะนำ เงินสด โอนธนาคาร พร้อมเพย์ เช็ค · หมายเหตุ · แล้วกดปุ่มบันทึกเต็มความกว้าง

ทั้งเบิกล่วงหน้าและจ่ายเงินถูกหักออกจากยอดสุทธิของเดือนนั้น ทำให้ช่อง คงเหลือ บอกว่ายังต้องจ่ายอีกเท่าไร และพนักงานเห็นรายการเดียวกันในหน้า เงินที่เบิก ของ LINE ทันที

### หน้า รายการค่าใช้จ่าย

ดูรายละเอียดที่หัวข้อ รายการค่าใช้จ่ายที่พนักงานส่ง ด้านบน

**วงจรของเงินค่าใช้จ่าย** พนักงานสำรองจ่าย → ส่งรายการพร้อมรูปใบเสร็จ → ผู้ดูแลอนุมัติ → ยอดบวกเข้ายอดสุทธิของเดือนนั้นอัตโนมัติ → พอคืนเงินจริงให้บันทึกเป็นรายการ **จ่ายเงิน** ในหน้า เงินที่เบิก → ยอดคงเหลือหักกลบกลับมาพอดี
...
**รูปใบเสร็จไม่ได้อยู่ในไฟล์ dump** เก็บเป็นไฟล์จริงใน volume `timework_uploads` ถ้าสำรองแต่ฐานข้อมูล เวลากู้คืนจะได้รายการค่าใช้จ่ายครบแต่รูปหายทั้งหมด ต้องสำรองแยกด้วย

```bash
cd /home/aon/timework-line
docker compose exec -T api tar -cf - -C /app uploads > backups/uploads-$(date +%F).tar
```
...
```bash
cd /home/aon/timework-line
docker compose exec -T api tar -xf - -C /app < backups/uploads-ชื่อไฟล์.tar
```
